...
void		split_room(t_all_data *main_uk, char *line, int8_t flags)
{
	...
	if (*line == 'L')
		ft_error("L CAN'T BE FIRST LETTER IN ROOM NAME", main_uk);
	if (!(check_space(line)))
		ft_error("NOT CORRECT COUNT OF SPACE IN ROOM", main_uk);
	if (((s = ft_split_cw(line, ' ', &res)) != 3) && (del_spl(res, s)))
		ft_error("INCORRECT NB OF ARGUMENTS IN ROOM", main_uk);
	if (!ft_check_cord(res) && (del_spl(res, s)))
		ft_error("BAD CORDINATES", main_uk);
	if (ft_strchr(line, '-') && (del_spl(res, s)))
		ft_error("NO - IN ROOM NAME", main_uk);
	if (((s = check_same_r_c(&main_uk->rnext, res)) < 0))
		ft_error(s == -1 ? "SAME ROOM NAME UP" : "SAME ROOM CORD UP", main_uk);
	if (flags == 1)
...
void		split_links(t_all_data *main_uk, char *line)
{
	...
	if ((check_minus(line)) != 1)
		ft_error("NOT CORRECT COUNT OF SPACE IN LINES", main_uk);
	if (((s = ft_split_cw(line, '-', &res)) != 2) && (del_spl(res, s)))
		ft_error("INCORRECT NB OF ARGUMENTS IN LINES", main_uk);
	if (ft_strequ(res[0], res[1]) && (del_spl(res, s)))
		ft_error("Same rooms name in links", main_uk);
	if (check_links(main_uk->rnext, res) && (del_spl(res, s)))
		ft_error("BAD LINKS", main_uk);
	if (check_double_link(ft_srch_beg_nb(&main_uk->rnext, res[0]), res[1]))
		ft_error("SAME LINKS", main_uk);
	++(main_uk->fl_lines);
...
{
	if (*line == ' ')
		ft_error("STRING CAN'T START WIHT THE SPACE", main_uk);
	if (*line == 'L')
		ft_error("L CAN'T BE FIRST LETTER IN STRING", main_uk);
	if (ft_strchr(line, ' '))
